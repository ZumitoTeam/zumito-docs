...
The framework emits a `ready` event when the bot is fully initialized (database connected, Discord client ready, all modules loaded, slash commands refreshed, and API server started). You can listen to it with:
...
Place this file in `events/framework/` inside your module. See also the [`onAllReady()`](/guides/module-lifecycle#onallready) module hook.
