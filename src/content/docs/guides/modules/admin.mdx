...
3. A JWT (`admin_token`) is issued with 2-hour expiry.
4. All admin routes validate this token via `AdminAuthService`.
...
| `AdminAuthService` | Validate JWT admin tokens and check superadmin status. |
